...
public class ConditionalRandomFieldMultipleEvent
{
    String tagFile;
    String templateFile;
    String corpusFile;
    ...
    public ConditionalRandomFieldMultipleEvent(final String confFile) {
        this.tagFile = null;
    ...
    public ConditionalRandomFieldMultipleEvent(final String tagFile, final String templateFile, final String corpusFile, final String parameterFile, final int numberOfColumn, final int learningTimes, final double errorLimit) {
        this.tagFile = null;
        this.templateFile = null;
        this.corpusFile = null;
        this.parameterFile = null;
    ...
    public void init() {
        ...
        if (this.isDebug) {
            String key2 = null;
            final Iterator<String> it = this.labels.keySet().iterator();
            while (it.hasNext()) {
                key2 = it.next();
                System.out.println(String.valueOf(key2) + " " + this.labels.get(key2));
            }
        }
        try {
            ...
            try {
                ...
                while ((line = br.readLine()) != null) {
                    if (!line.equals("") && !line.equals(" ")) {
                        line = line.trim();
                        if (isFirstLine) {
                            this.tagset = this.labels.get(line);
                            isFirstLine = false;
                        }
                        else {
                            tokens = line.split("\\s+");
